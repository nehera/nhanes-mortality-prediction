---
title: "NHANES Activity Time Series to Images"
author: "Aidan Neher"
format: html
editor: visual
---
...
Let's start by removing objects we don't care about (e.g. multi-level data) for now and looking at one hour of a subject's aggregated tseries:

```{r}

# rm(act_cnt_ml, data_analysis, mort_cov, PAXINTEN_C, PAXINTEN_D)

AC <- matrix(act_cnt_sl$AC, nrow = nrow(act_cnt_sl), byrow = TRUE)

n_minutes <- 60

sub1 <- data.frame(minute=1:n_minutes, activity=AC[1, 1:60])

ggplot(sub1, aes(x = minute, y = activity)) + geom_line()
...
AC_long <- AC_df %>% 
  ...
  mutate(minute = str_remove(minute, "V") %>% as.integer()) %>%
  mutate(bin_index = (minute - 1) %/% 6 + 1)

# Calculate the mean for each bin
AC_binned <- AC_long %>%
  group_by(observation_id, bin_index) %>%
  summarise(
...
# Re-split activity data into waves C and D
ids_C <- unique(PAXINTEN_C$SEQN)
index_C <- which(act_cnt_sl$SEQN %in% ids_C)
AC_df <- AC_df %>%
  select(-observation_id) %>%
  mutate(SEQN = act_cnt_sl$SEQN)
act_bin_sl_C <- AC_df[index_C, ]
act_bin_sl_D <- AC_df[-index_C, ]

# Store single-level averaged and 6-minute binned data

write.csv(act_bin_sl_C, file = "data/act_bin_sl_C.csv")
write.csv(act_bin_sl_D, file = "data/act_bin_sl_D.csv")

```

Check mortality missingness/ censoring:

```{r}

Mortality_2015_D$mortstat
intersect(Mortality_2015_D$SEQN[which(!is.na(Mortality_2015_D$mortstat))], act_bin_sl_D$SEQN) %>% length
