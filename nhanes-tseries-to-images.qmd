...
AC_long <- AC_df %>% 
  ...
  mutate(minute = str_remove(AC_long$minute, "V") %>% as.integer()) %>%
...
Now we explore and save our binned single-level activity data: We start by visualizing 3 subjects' binned tseries and then store both this subsample of subjects and the entirety of the binned data.frame. 
...
write.csv(AC_wide[1:3, ], file = "binned_sl_AC_sample.csv")
write.csv(AC_wide, file = "binned_sl_AC.csv")
